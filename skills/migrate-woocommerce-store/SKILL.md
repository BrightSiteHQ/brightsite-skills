...
## Onboard the account first

A brand-new account has `is_onboarded` false, and the first visit to its
dashboard runs the setup wizard, which seeds the starter template over whatever
you imported. Call `complete_onboarding` before writing any content.
...
then `add_products_to_collection`, then `set_collection_status(active)`.
**A collection is created as draft and holds no products** — miss either call and
its storefront page is empty or unreachable.
...
4. Rewrite the `src` in the description to the new BrightSite media URL.
   `complete_upload` returns them (`md_url` suits in-body images) — product
   descriptions are plain HTML and need a real absolute URL; there is no
   media-id reference form for description HTML.
...
## Step 5: rebuild the pages — use the cloning skill

Do not author the pages from this skill. Run **clone-site-to-brightsite** on the
store's real content pages (home, about, contact — whatever the shop actually
has). It measures the source with `getComputedStyle()`, rehosts the assets, and
diffs its output against the original, which is the only way the result looks
like the shop it replaced.
...
templates. On most shops that leaves three or four real pages.

Hand the cloning skill the **media id map** from Step 4, so it reuses the images
you already rehosted instead of uploading them twice.

### Where products can and cannot be listed

`product-grid` reads a `products` assign that the renderer only fills on a
**collection page** (`/collections/<slug>`). Embedding `component("product-grid")`
on an ordinary CMS page renders "No products yet." no matter how many active
products exist.

So: link the homepage to the collection, or feature products with hand-authored
markup. Do not promise a homepage product grid.
...
The MCP endpoint allows **100 requests per minute**. A shop with 50 images needs
roughly 150 calls between `request_upload`, the PUT and `complete_upload`, so a
straight loop hits the ceiling and starts failing with HTTP 429. Back off
exponentially and retry rather than dropping the asset.
